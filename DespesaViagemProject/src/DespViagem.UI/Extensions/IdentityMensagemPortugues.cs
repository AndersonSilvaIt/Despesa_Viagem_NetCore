...
namespace DespViagem.UI.Extensions
{
	public class IdentityMensagemPortugues : IdentityErrorDescriber
	{
		...
		public override IdentityError LoginAlreadyAssociated() { return new IdentityError { Code = nameof(LoginAlreadyAssociated), Description = $"Já existe um usuario com este login." }; }
		...
		public override IdentityError PasswordRequiresNonAlphanumeric() { return new IdentityError { Code = nameof(PasswordRequiresNonAlphanumeric), Description = $"As senhas devem conter ao menos um caracterer.' )." }; }
		...
		public override IdentityError PasswordRequiresLower() { return new IdentityError { Code = nameof(PasswordRequiresLower), Description = $"As senhas devem conter ao menos um caractere em caixa baixa." }; }
		public override IdentityError PasswordRequiresUpper() { return new IdentityError { Code = nameof(PasswordRequiresUpper), Description = $"As senhas devem conter ao menos um caractere em caixa alta." }; }
	}
}
